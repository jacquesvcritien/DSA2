...
public class FileOperation {
    ...
    public static void populateFrequencies(String filename) throws IOException, CharacterNotSupportedException {
        ...
        while(asciiCode != -1)
        {
            ...
            currentVal = HuffmanCoding.frequencies.get(character);
            //add 1 to it
            HuffmanCoding.frequencies.put(character, currentVal+1);
            //read next char
        ...
        for (Map.Entry<Character, Integer> entry : HuffmanCoding.frequencies.entrySet()) {
            newNode = new Node(entry.getValue(), entry.getKey());
            HuffmanCoding.sortedChars.add(newNode);
        }
    }
    ...
    public static void populateInitFrequencies(String filename) throws IOException, CharacterNotSupportedException {
        ...
        while(asciiCode != -1)
        {
            ...
            if(HuffmanCoding.frequencies.containsKey(character)) {
                currentVal = HuffmanCoding.frequencies.get(character);
                HuffmanCoding.frequencies.put(character, currentVal + 1);
            }
            ...
            {
                HuffmanCoding.frequencies.put(character, 1);
            }
        ...
        HuffmanCoding.sortedChars = new PriorityQueue<Node>(HuffmanCoding.frequencies.size(), new NodeComparator());
